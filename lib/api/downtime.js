...
function update(downtimeId, properties, callback){
    ...
    if(typeof properties === "object"){
        ...
        if(properties.monitor_tags){
            params.body.monitor_tags = properties.monitor_tags;
        }
        if(properties.monitor_id){
            params.body.monitor_id = parseInt(properties.monitor_id);
        }
        if(properties.timezone){
            params.body.timezone = properties.timezone;
        }
        if(properties.recurrence){
            var until_date = properties.recurrence.until_date;
            var until_occurrences = properties.recurrence.until_occurrences;
            var recurrence = Object.assign({}, properties.recurrence);
            if (!until_occurrences) {
                delete recurrence.until_occurrences;
            }
            if (!until_date) {
                delete recurrence.until_date;
            }

            params.body.recurrence = recurrence;
        }
    }
